...
Every preset names every macro, `event_rate` included. The arrival rate used to
be part of `intensity`, so each preset already implied one; the values here are
the rates these presets have always produced, carried across unchanged when the
rate became a knob of its own.
"""
